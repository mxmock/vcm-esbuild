...
const VillaDetails = ({ data }) => {
  ...
  const services = villa.features.filter((f) => byCat(f, 3)).map(mapFeature);
  const globalView = villa.features.filter((f) => byCat(f, 1)).map(mapFeature);
  const mainFeatures = villa.features.filter((f) => byCat(f, 6)).map(mapFeature);
  const additionnals = villa.features.filter((f) => byCat(f, 4)).map(mapFeature);
